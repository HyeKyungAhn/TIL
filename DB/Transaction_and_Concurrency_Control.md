...
3. 사용자 A가 두 번쨰 검색을 함. 총 6행의 결과를 얻음
    ...
    사용자 A는 총 두 번의 검색을 할 때 데이터의 행 수가 달랐음. 데이터를 조회할 때 데이터가 사라질 수도, 새로 나타날 수도 있어서 Phantom Read라고 이름을 붙임. MySQL에서는 MVCC를 사용하여 동시성 제어를 하기 때문에 phantom read가 발생하지 않는다고 합니다.
